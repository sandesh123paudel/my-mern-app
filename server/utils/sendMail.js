import { ADMIN_INQUIRY_NOTIFICATION_TEMPLATE, CUSTOMER_INQUIRY_CONFIRMATION_TEMPLATE } from "../config/emailTemplate.js";
...
export const sendAdminInquiryNotification = async (inquiryData) => {
  try {
    ...
    let emailContent = ADMIN_INQUIRY_NOTIFICATION_TEMPLATE
      .replace(/{{name}}/g, inquiryData.name)
      ...
      .replace(/{{adminDashboardUrl}}/g, process.env.ADMIN_DASHBOARD_URL || 'https://www.mulchowkkitchen.com.au');

    // Handle optional message
    if (inquiryData.message) {
      emailContent = emailContent.replace(/{{#if message}}/g, '').replace(/{{\/if}}/g, '');
    ...
    const mailOptions = {
      ...
      subject: `🔔 New Inquiry: ${inquiryData.name} - ${formatDate(inquiryData.eventDate)}`,
      html: emailContent,
    };
...
export const sendCustomerInquiryConfirmation = async (inquiryData) => {
  try {
    // Replace template variables
    let emailContent = CUSTOMER_INQUIRY_CONFIRMATION_TEMPLATE
      ...
      .replace(/{{serviceType}}/g, inquiryData.serviceType)
      .replace(/{{submittedAt}}/g, formatDate(inquiryData.createdAt))
      .replace(/{{companyName}}/g, process.env.COMPANY_NAME || 'MC Catering Services')
      .replace(/{{supportEmail}}/g, process.env.EMAIL)
      .replace(/{{supportPhone}}/g, process.env.SUPPORT_PHONE || '+61 XXX XXX XXX')
      .replace(/{{businessHours}}/g, process.env.BUSINESS_HOURS || 'Mon-Fri 9:00 AM - 6:00 PM')
      .replace(/{{websiteUrl}}/g, process.env.ADMIN_DASHBOARD_URL || 'https://www.mulchowkkitchen.com.au')
    ...
    const mailOptions = {
      ...
      subject: `✅ Inquiry Confirmation - ${process.env.COMPANY_NAME || 'MC Catering Services'}`,
      html: emailContent,
    };
