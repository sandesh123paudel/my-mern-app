...
const AdminInquiries = () => {
  ...
  const [showAddModal, setShowAddModal] = useState(false); // Pagination state

  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(5); // Changed from 10 to 5
  const [totalItems, setTotalItems] = useState(0);
  const [totalPages, setTotalPages] = useState(0); // Debounced search

  const [debouncedSearchTerm, setDebouncedSearchTerm] = useState(""); // Debounce search term
  ...
  }, [searchTerm]); // Fetch inquiries with pagination and filters
  ...
  useEffect(() => {
    fetchInquiries();
  }, [fetchInquiries]); // Reset to first page when filter changes
